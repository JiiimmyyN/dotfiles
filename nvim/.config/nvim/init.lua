...
-- Enable mouse mode, can be useful for resizing splits for excc!
vim.o.mouse = "a"
...
require("lazy").setup({
  ...
  },
  {
    ...
    opts = function()
      ...
      local function save_condition(bufnr)
        local ft = vim.bo[bufnr].filetype -- same as getbufvar(buf, "&filetype")
    ...
    config = function()
      vim.api.nvim_create_autocmd("LspAttach", {
        ...
        callback = function(event)
          ...
          map("<C-a>", vim.lsp.buf.code_action, "[G]oto Code [A]ction", { "n", "x", "v" })

          map("<Esc>[27;4;9", vim.lsp.buf.code_action, "[G]oto Code [A]ction", { "n", "v" })
          ...
          map("gsh", vim.lsp.buf.signature_help, "Show Signature Help", { "n", "i" })
  ...
  },
  {
    ...
    opts = {
      filesystem = {
        filtered_items = {
          visible = false, -- This is what you want to set to true to see hidden files
          hide_dotfiles = false,
